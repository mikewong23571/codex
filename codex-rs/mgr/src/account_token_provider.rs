...
async fn load_from_auth(
    ...
    let auth = CodexAuth::from_auth_storage(&account_home, AuthCredentialsStoreMode::File)
        .with_context(|| format!("loading auth from {account_home:?}"))?
        .with_context(|| format!("missing auth for account {account_id:?}"))?;
    ...
    if expires_at_ms.saturating_sub(now_ms) <= safety_ms {
        ...
        token_data = auth.get_token_data().await.with_context(|| {
